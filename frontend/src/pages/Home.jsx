function Home({ onNavegar }) {
  return (
    <div>
      {/* Hero */}
      <section
        className="rounded-3xl p-8 sm:p-12 text-center mb-10 relative overflow-hidden"
      ...
      >
        ...
        <h2 className="text-3xl sm:text-5xl font-bold text-pink-700 mb-4 leading-tight">
        ...
        <div className="flex gap-3 sm:gap-4 justify-center flex-wrap">
          <button
            ...
            className="bg-pink-600 text-white px-6 sm:px-8 py-3 rounded-full text-base sm:text-lg hover:bg-pink-700 transition shadow-lg"
          >
            Ver Catalogo
          </button>
          <button
            onClick={() => onNavegar("contato")}
            className="bg-white text-pink-600 border-2 border-pink-300 px-6 sm:px-8 py-3 rounded-full text-base sm:text-lg hover:bg-pink-50 transition"
          >
            Fale Conosco
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div
              key={cat.nome}
              onClick={() => onNavegar("catalogo")}
              className={`${cat.cor} rounded-2xl shadow p-5 sm:p-6 text-center transition cursor-pointer`}
            >
              <div className="text-4xl sm:text-5xl mb-3">{cat.emoji}</div>
              <div className="font-semibold text-gray-700 text-sm sm:text-base">{cat.nome}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-green-50 rounded-3xl p-8 text-center">
        ...
        <button
          onClick={() => window.open("https://wa.me/5546991360839", "_blank")}
